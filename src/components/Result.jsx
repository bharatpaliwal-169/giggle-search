...
export function Results(){
  ...
  switch (location.pathname) {
      ...
      return (
        <div className='flex flex-wrap justify-between space-y-6 sm:px-56'>
          {results?.results?.map(({url,title,description,position})=>(
              <div key={position} className='w-full md:w-2/5'>
    ...
    //       {results?.entries?.map(({ id, urls, source, title }) => (
    //         <div key={id} className="md:w-2/5 w-full ">
    //           <a href={urls?.[0].href} target="_blank" rel="noreferrer " className="hover:underline ">
    ...
    //           <ReactPlayer url={video.additional_urls?.[0].href} controls width="355px" height="200px" />
    //         </div>
    //       ))}
    //     </div>
    //   );
